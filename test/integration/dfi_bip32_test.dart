import 'package:bs58check_dart/bs58check.dart';
...
void main() {
  group('defichain-dart (BIP32)', () {
    ...
    test('can export a BIP32 xpub', () {
      ...
      expect(string, 'xpub661MyMwAqRbcEgr29GBcXvmd3dekivdL4BSMJwfNdtu17Z3saFuWu2oBBLCF5dA2Afav6EALJy9rCtAtsX4dLkLduJo29tq9KSFAszJrsfy');
    });
    test('can create a BIP32, defichain, account 0, external address', () {
      const path = "m/0'/0/0";
      final root = bip32.BIP32.fromSeed(HEX.decode('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd'), _DEFICHAIN);
    ...
    test('can use BIP39 to generate BIP32 addresses', () {
      ...
      expect(getAddress(root.derivePath("m/0'/0/0"), networks.defichain_testnet), '1AVQHbGuES57wD68AJi7Gcobc3RZrfYWTC');
      expect(getAddress(root.derivePath("m/0'/0/1"), networks.defichain_testnet), '1Ad6nsmqDzbQo5a822C9bkvAfrYv9mc1JL');
      // change addresses
      expect(getAddress(root.derivePath("m/0'/1/0"), networks.defichain_testnet), '1349KVc5NgedaK7DvuD4xDFxL86QN1Hvdn');
      expect(getAddress(root.derivePath("m/0'/1/1"), networks.defichain_testnet), '1EAvj4edpsWcSer3duybAd4KiR4bCJW5J6');
    });
    ...
    test("can import recovery phrase", () {
      ...
      expect(wif, "tprv8ZgxMBicQKsPd9Gff9E9fvhL5SDCLdKbjPbaREPyjLk743Sry9nAmESmaWwijZuGqer1Q4rG1SaUhc7XHvFg6y44z6JaKmTeHyJgNQism1U");
    });
  });
}
